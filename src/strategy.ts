...
export interface Bid {
  timestamp: number;
  amount: number;
  bidder: string; // 'self' or 'opponent_N'
}
...
export interface StrategyConfig {
  maxBid: number;
  targetValue: number;
  aggressiveness: number; // 0-1, how much to fight for Sente
  sniping: boolean; // Whether to use last-second bidding
  snipeWindow: number; // seconds before end to place snipe bid
}
...
export class WarOnArtStrategy {
  ...
  private calculateBidPressure(state: AuctionState): number {
  ...
  private calculatePriceAcceleration(bids: Bid[]): number {
  ...
  private calculateSenteValue(state: AuctionState): number {
  ...
  private endGameStrategy(
    ...
    if (valueRatio < 1.0) {
      const finalBid = Math.min(
        this.config.maxBid,
        state.currentPrice + state.currentPrice * 0.1
      );
  ...
  private calculateStrategicIncrement(state: AuctionState, senteValue: number): number {
   ...
   */
  updateStrategy(feedback: { won: boolean; finalPrice: number; efficiency: number }): void {
