    ...
    try {
        switch (name) {
            case 'initialize_auction': {
                const { auctionId, auctionUrl, maxBid, estimatedValue, aggressiveness = 0.7, sniping = false, snipeWindow = 10, headless = true, } = args;
            ...
            case 'check_auction_state': {
                ...
                const newState = await auction.agent.getAuctionState(auctionId, auction.config.targetValue);
            ...
            case 'place_bid': {
                const { auctionId, amount, force = false } = args;
                const auction = activeAuctions.get(auctionId);
                if (!auction) {
                    throw new Error(`Auction ${auctionId} not found. Initialize first.`);
                }
            ...
            case 'auto_bid': {
                const { auctionId, checkInterval = 30, duration } = args;
                const auction = activeAuctions.get(auctionId);
                if (!auction) {
                    throw new Error(`Auction ${auctionId} not found. Initialize first.`);
                }
            ...
            case 'screenshot': {
                const { auctionId, path = './auction_screenshot.png' } = args;
                const auction = activeAuctions.get(auctionId);
                if (!auction) {
                    throw new Error(`Auction ${auctionId} not found.`);
                }
